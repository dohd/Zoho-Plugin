...
@section('content')
            ...
            <div class="card-content p-2">
                ...
                <div class="tab-content" id="myTabContent">
                    ...
                    <div class="tab-pane fade show active" id="info" role="tabpanel" aria-labelledby="info-tab">
                        <div class="m-2">
                            @foreach ($invoiceCols as $row)
                                <div class="form-group row mb-2">
                                    @foreach ($row as $cell)
                                        <div class="col-md-3">
                                            @if ($cell == 'payroll_no')
                                                <label for="{{ $cell }}">PF No.</label>
                                                {{ Form::text($cell, $invoice->$cell, ['class' => 'form-control', 'disabled' => 'disabled']) }}
                                            @else
                        ...
                        <div class="row m-2">
                            <div class="col-md-10">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($invoice->documents as $i => $doc)
                                            <tr>
                                                <td>{{ $i+1 }}</td>
                                                <td>{{ $doc->doc_type }}</td>
                                                <td><a target="_blank" href="{{ route('invoices.document_download', $doc) }}" class="ms-1 dn-link"><u>{{ $doc->origin_name }}</u></a></td>
                                                <td>{{ $doc->caption }}</td>
                                                <td></td>
                                            </tr>
                                        @endforeach
    ...
    @include('invoices.document_modal')
@stop
